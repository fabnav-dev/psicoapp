  ...
  return (
    <div style={{ minHeight:'100%', background:t.headerGrad, display:'flex', alignItems:'flex-start', justifyContent:'center', padding:'24px 18px' }}>
      <div style={{ maxWidth:480, width:'100%', textAlign:'center', color:'#fff', margin:'auto 0' }} className="fade">
        ...
        <div style={{ fontSize:11, color:'rgba(255,255,255,0.5)', marginTop:24 }}>Elaborado por <span style={{ color:'rgba(255,255,255,0.8)', fontWeight:700 }}>✦ Rumbo</span></div>
...
function AppHeader({ t, role, onLogout, onSwitch, notifCount, notifs, revisiones }){
  ...
  if(role==='equipo'){
    ...
    (revisiones||[]).filter(r=>r.estado==='archivado').filter(r=>{ const f=fechaRev(r); if(!(f && (new Date()-f)<=7*864e5)) return false; try{ return !lsGet('psico_impresos_v1',[]).includes(r.id); }catch(e){ return true; } }).forEach(r=>pend.push({ ic:'check', txt:'Documento completo · imprimir y archivar', sub:`${r.estNombre} · ${r.planNombre}` }));
                  ...
                  <div style={{ background:t.card, border:`1px solid ${t.border}`, borderRadius:t.radius, padding:'13px 15px' }}>
                    ...
                    {completo && (
                      <div style={{ marginTop:12, background:'#E2F3EC', border:'1px solid #A8D8C0', borderRadius:10, padding:'11px 13px', textAlign:'center' }}>
                        ...
                        <button onClick={()=>{ imprimirOficial(plan, est, marcadas, adecSet, '17 jun 2026', resp, director, equipo, (obs||(rev&&rev.obs)||''), rev); try{ const im=lsGet('psico_impresos_v1',[]); if(rev&&rev.id&&!im.includes(rev.id)){ lsSet('psico_impresos_v1',[...im,rev.id]); window.dispatchEvent(new Event('impresos-change')); } }catch(e){} onToast('Documento impreso y archivado'); }} style={{ width:'100%', padding:11, background:t.primary, color:'#fff', border:'none', borderRadius:10, fontSize:12.5, fontWeight:700, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap:7 }}><Icon k="print" c="#fff" s={16} />Imprimir y archivar</button>
                      </div>
                    )}
                  </div>
                );
